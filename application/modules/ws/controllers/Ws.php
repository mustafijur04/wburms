...
class Ws extends REST_Controller
{
    ...
    function api_post()
    {
        ...
        switch ($code) {
            ...
            case WS_SURVEY_LIST:
                $this->survey_api();
                break;
            case WS_SURVEY_SUBMIT:
                $this->survey_submit_api();
                break;
            case WS_DISTRICT_LIST:
                $this->get_district_list_api();
                break;
            case WS_block_LIST:
                $this->get_block_list_api();
                break;
            case WS_GP_LIST:
                $this->get_gp_list_api();
                break;
            case WS_ROAD_NAME_LIST:
                $this->get_name_list_api();
                break;
            default:
                break;
        }
    ...
    function survey_submit_api()
    {
        ...
        // WS_DATA Structure Check
        if (!isset($request['WS_DATA'][0])) {
            $this->response([
                "status"  => false,
                "message" => "Invalid API format. WS_DATA missing."
            ], REST_Controller::HTTP_BAD_REQUEST);
            return;
        }

        $data = $request['WS_DATA'][0];

        // Required field validation
        if (empty($data['roads_id'])) {
            $this->response([
                "status"  => false,
                "message" => "roads_id is required"
            ], REST_Controller::HTTP_BAD_REQUEST);
            return;
        }
        $result = $this->ws_model->save_survey_log($data);
    ...
    function get_block_list_api()
    {
        $input = json_decode(file_get_contents("php://input"), true);
        $data  = $input['WS_DATA'][0] ?? [];
        ...
        $block_list = $this->common->get_block_list($data['district_id']);
    ...
    function get_gp_list_api()
    {
        ...
        $gp_list = $this->common->get_gp_list($data['block_id']);
    ...
    {
        $input = json_decode(file_get_contents("php://input"), true);
        $input = $input['WS_DATA'][0] ?? [];

        if (empty($input['district_id']) || empty($input['block_id']) || empty($input['name_id'])) {
            $this->response([
                'status' => false,
                'message' => 'district_id, block_id, name_id are required'
            ], 400);
            return;
        }
