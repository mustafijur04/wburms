...
class Ws_Model extends CI_Model
{
    ...
    function get_survey_list($district_id, $block_id = 0, $gp_id = 0, $name_id = 0)
    {
        // ---------------- If name selected → return that record only ----------------
        if ($name_id > 0) {
        ...
        // ---------------- Normal Search (district/block/gp filter) ----------------
        $this->db->select('
        s.id,
        d.name AS district,
        b.name AS block,
    ...
    ');
        ...
        // Apply GP filter only if selected
        if ($gp_id > 0) {
            $this->db->where("FIND_IN_SET($gp_id, s.gp_id) !=", 0);
        }

        // // Agency filter
        // $this->db->where_in('s.agency', ['KMDA', 'MED']);
    ...
    function get_name_list($block_id, $gp_id = 0)
    {
        ...
        if ($gp_id > 0) {
            $gp_id = (int)$gp_id;
            $this->db->where("gp_id IS NOT NULL AND gp_id != ''");
            $this->db->where("FIND_IN_SET($gp_id, gp_id) >", 0);
        }
    ...
    function filter_with_agencys()
    {
        ...
        switch ($session['role_id']) {
            case 13:
                $this->db->where("TRIM(UPPER(s.agency)) =", "MED");
                break;
            case 14:
                $this->db->where("TRIM(UPPER(s.agency)) =", "KMDA");
                break;
            case 15:
                $this->db->where("TRIM(UPPER(s.agency)) =", "KMC");
                break;
        }
    ...
    function save_survey_log($data)
    {
        // Prepare log data
        $log = [
            'roads_id' => $data['roads_id'],
            'survey_date' => $data['survey_date'] ?? null,
            'is_weather_executable' => $data['is_weather_executable'] ?? null,
            'not_executable_reason' => $data['not_executable_reason'] ?? null,
            'other_reason' => $data['other_reason'] ?? null,
            'other_scheme_name' => $data['other_scheme_name'] ?? null,
            'current_work_status' => $data['current_work_status'] ?? null,
            'rsp_id' => $data['rsp_id'] ?? null,
            'aafs_no' => $data['aafs_no'] ?? null,
            'aafs_date' => $data['aafs_date'] ?? null,
            'fund_sanctioned' => $data['fund_sanctioned'] ?? null,
            'head_of_account' => $data['head_of_account'] ?? null,
            'executable_work_type' => $data['executable_work_type'] ?? null,
            'executable_road_type' => $data['executable_road_type'] ?? null,
            'executable_bt_length' => $data['executable_bt_length'] ?? null,
            'executable_cc_length' => $data['executable_cc_length'] ?? null,
            'executable_paver_length' => $data['executable_paver_length'] ?? null,
            'impl_engineer_name' => $data['impl_engineer_name'] ?? null,
            'impl_engineer_designation' => $data['impl_engineer_designation'] ?? null,
            'impl_engineer_contact' => $data['impl_engineer_contact'] ?? null,
            'ulb_engineer_name' => $data['ulb_engineer_name'] ?? null,
            'ulb_engineer_designation' => $data['ulb_engineer_designation'] ?? null,
            'ulb_engineer_contact' => $data['ulb_engineer_contact'] ?? null,
            'created' => date('Y-m-d H:i:s'),
            'status' => 1,
        ];

        $this->db->insert('roads_survey_log', $log);

        if ($this->db->affected_rows() > 0) {

            $update = [
                'survey_date' => $log['survey_date'],
                'survey_status' => $log['status'],
            ];

            $this->db->where('id', $data['roads_id']);
            $this->db->update('roads', $update);
    ...
    function get_scheme_count($data)
    {
    ...
    function get_scheme_wo_details($data)
    {
        ...
        // $this->db->select('*');
        $this->db->where(array(
            'ssm_id' => $data->project_id,
            // 'survey_status' => 6,
            'isactive' => 1
        ));
        $query = $this->db->get(SSM_WO);
        // return $query->result();
        return $query->row();
