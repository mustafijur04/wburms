 ...
 * @author		Suvendu Mondal (suvendumandal742@gmail.com)
 * @since		Version 4.0,[Created: 01-Oct-2025]
 */
// ------------------------------------------------------------------------
defined('BASEPATH') or exit('No direct script access allowed');

class ROADS extends MX_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('roads_model', 'model');
    ...
    function overview()
    {
        ...
        if ($session['role_id'] < 10) {
            $this->session->set_userdata('menu', 145);
        }
        $this->data['role_id'] = $session['role_id'];
        $this->data['district_chart'] = json_encode($this->model->get_district_wise_count());
        $this->data['dashboard_count'] = json_encode($this->model->get_dashboard_count());
        // echo "<pre>";
        // print_r($this->data['dashboard_count']);exit;
        $this->data['tender_and_wo_count'] = json_encode($this->model->get_tender_and_wo_count());
        $this->data['title'] = 'Rural Roads(2025)';
        $this->data['heading'] = 'Overview - Rural Roads(2025)';
    ...
    function index()
    {
        ...
        $this->data['survey'] = json_encode($this->model->get_survey_list($selected['district_id']), $selected['block_id']);
        // echo "<pre>";
        // print_r($this->data);exit;
        $this->data['title'] = 'Pathashree Urban';
        $this->data['heading'] = 'Master - Pathashree Urban';
        $this->data['subheading'] = 'Pathashree Urban';
...
//    this is manage only Bt road and cc road

    public function import_all_bc()
    {
        $limit = 500;
        $offset = 0;
        $total_updated = 0;
        ...
        while (true) {
            $rows = $this->db->limit($limit, $offset)->get('roads')->result();
            if (empty($rows)) break;

            foreach ($rows as $row) {
                $update_data = [];

                $road_type = trim($row->road_type ?? '');

                if ($road_type === 'Bituminious(Tar)Road') {
                    $update_data['bt_length'] = $row->length ?? 0;
                    $update_data['cc_length'] = 0; // auto set CC = 0
                } elseif ($road_type === 'Concrete Road') {
                    $update_data['cc_length'] = $row->length ?? 0;
                    $update_data['bt_length'] = 0; // auto set BT = 0
                }

                if (!empty($update_data)) {
                    $this->db->where('id', $row->id)->update('roads', $update_data);
        ...
        echo "Updated {$total_updated} records successfully!";
    }
    ...
    function entry($id = 0)
    {
        ...
        $selected = array(
            ...
            'new_road_type' => '',
            'new_length' => '',
            'bt_length' => !empty($bt_length) ? $bt_length : 0,
            'cc_length' => !empty($cc_length) ? $cc_length : 0,
            'paver_length' => !empty($paver_length) ? $paver_length : 0,
            'length' => '',
            'work_type' => '',
            'cost' => '',
            'survey_status' => 0,
        ...
        if ($id > 0) {
            ...
            $selected = array(
                ...
                'new_road_type' => $row->new_road_type,
                'new_length' => $row->new_length,
                'bt_length' => $row->bt_length,
                'cc_length' => $row->cc_length,
                'paver_length' => $row->paver_length,
                'cost' => $row->cost,
                'survey_status' => $row->survey_status,
                'isapproved' => $row->survey_status < 6 ? 0 : 1,
                'admin_no' => isset($row->admin_approval_no) ? $row->admin_approval_no : '',
                'admin_date' => isset($row->admin_approval_date) ? date('d/m/Y', strtotime($row->admin_approval_date)) : ''
            );
            $subheading = $row->name;
        }
        $district_list = $this->common->get_district_list();
        ...
        // print_r($this->data['ac']);exit;
        $this->data['block'] = json_encode($block_list);
        $this->data['gp'] = json_encode($gp_list);
        $this->data['survey'] = json_encode($survey_list);
        $this->data['title'] = 'Pathashree Urban';
        $this->data['heading'] = 'Master Entry - Pathashree Urban';
    ...
    function update_proposed_length()
    {
        // Get POST data
        $id = $this->input->post('id');
        $proposed_length = $this->input->post('proposed_length');
        // print_r($id, $proposed_length);exit;

        if ($id && $proposed_length !== null) {
            $update = $this->model->update_proposed_length($id, $proposed_length);
    ...
    function save_atr_review()
    {
        ...
        if ($id > 0) {
            $this->session->set_flashdata('message', array('type' => 'success', 'message' => 'Data addded successsmully'));
            redirect('ssm/qm_atr');
        }
    }
    ...
    function survey()
    {
        ...
        $this->data['block'] = json_encode($block_list);
        $this->data['survey'] = json_encode($this->model->get_survey_pending_list($selected['district_id'], $selected['block_id'], $selected['ac_id']));
        $this->data['title'] = 'Survey';
        $this->data['heading'] = 'Scheme Inbox - Rural Roads(2025)'; //'Pending Survey';
    ...
    function survey_entry($id)
    {
        ...
        if ($id > 0) {
            ...
            $selected = array(
                ...
                'work_type' => $row->work_type,
                'status' => $row->status,
                'survey_estimated_doc' => $row->survey_estimated_doc
            );
        }
        $this->data['selected'] = json_encode($selected);
        $this->data['title'] = 'Survey - Rural Roads(2025)';
        $this->data['heading'] = 'Survey - Rural Roads(2025)';
    ...
    function survey_save()
    {
        ...
        if ($this->model->survey_save($data)) {
            $this->survey_upload($data['id']);
            redirect('roads/survey');
        } else {
    ...
    function survey_upload($id)
    {
        // print_r($id);
        // exit;
        $path = 'uploads/roads/survey_estimated';
        if (!file_exists('./' . $path)) {
        ...
        if ($this->upload->do_upload('userfile')) {
            ...
            $new_file_name = $id . '_' . time() . $upload_data['file_ext'];
            $new_file_path = $path . '/' . $new_file_name;

            rename($upload_data['full_path'], './' . $new_file_path);
            $this->db->where('id', $id);
            $this->db->update(ROADS, ['survey_estimated_doc' => $new_file_path]);
        } else {
            $error = $this->upload->display_errors();
            // Optionally, handle or log the error
            log_message('error', 'Survey upload failed: ' . $error);
        }
    }

    function survey_upload_manage($id)
    {
        $path = 'uploads/roads/survey_estimated';
        if (!file_exists('./' . $path)) {
            mkdir('./' . $path, 0777, true);
        ...
        $upload_data = $this->upload->data();
        $new_file_path = $path . '/' . $upload_data['file_name'];
    ...
    public function survey_vec_save()
    {
        $data = $this->input->get();
        // print_r($data);exit;
        $this->model->survey_vec_save($data);
        echo json_encode(['status' => 'success']);
    }

    public function survey_dpr_save()
    {
        $data = $this->input->get();
        $this->model->survey_dpr_save($data);
        echo json_encode(['status' => 'success']);
    }

    public function survey_con_save()
    {
        $data = $this->input->get();
        $this->model->survey_con_save($data);
        echo json_encode(['status' => 'success']);
    }

    public function survey_est_save()
    {
        $data = $this->input->get();
        $this->model->survey_est_save($data);
        echo json_encode(['status' => 'success']);
    }
    ...
    function lot()
    {
        ...
        $district_list = $this->model->get_district_list();
        // print_r($district_list);exit;
        if (sizeof($district_list) == 1) {
            $selected['district_id'] = $district_list[0]->id;
        ...
        // print_r($this->data['lotno']);exit;
        $this->data['title'] = 'Scheme';
        $this->data['heading'] = 'Batch/Lot Inbox - Rural Roads(2025)'; // 'Scheme';
    ...
    function approval()
    {
        ...
        // print_r($this->data['approval']);exit;
        $this->data['title'] = 'Survey';
        $this->data['heading'] = 'Vetted Scheme Inbox - Rural Roads(2025)'; // 'Approval';
        $this->data['subheading'] = 'Scheme Details';
        $this->data['selected'] = json_encode($selected);
    ...
    {
        $data = $this->input->get();
        echo json_encode($this->model->get_survey_pending_list($data['district_id'], $data['block_id'], $data['ac_id']));
    }
    ...
    function admin_approval()
    {
        ...
        if (!$this->upload->do_upload('userfile')) {
            ...
            foreach ($result as $row) {
                $scheme_no++;
                $ref_no = str_pad($scheme_no, 5, '0', STR_PAD_LEFT) . '/' . $row->agency . '/' . $row->district . '/RASTASHREE4/2025';
                $input = array(
                    'scheme_no' => $scheme_no,
                    'ref_no' => $ref_no
                );
    ...
    function approved()
    {
        ...
        $this->data['ac'] = json_encode($this->model->get_ac_list());
        $this->data['block'] = json_encode($block_list);
        $this->data['approved'] = json_encode($this->model->get_approved_list($selected['district_id'], $selected['block_id'], $selected['ac_id']));
        $this->data['title'] = 'Scheme';
        $this->data['heading'] = 'Approved Scheme Inbox - Rural Roads(2025)';
        $this->data['subheading'] = 'Scheme Details';
        $this->data['selected'] = json_encode($selected);
    ...
    {
        $data = $this->input->get();
        echo json_encode($this->model->get_approved_list($data['district_id'], $data['block_id'], $data['ac_id']));
    }

    function print_lot($district_id, $lotno)
    {
        $this->data['title'] = 'Print';
        $this->data['heading'] = 'Print Lot - Rural Roads(2025)';
        $this->data['subheading'] = 'Print';
        $this->data['lotno'] = $lotno;
        $this->data['print'] = json_encode($this->model->print_lot($district_id, $lotno));
    ...
    {
        $data = $this->input->post();
        $this->model->return_to_prev($data['arr'], $data['msg']);
    ...
    function update_sa_lot_null()
    {
        $id = $this->input->post('id');

        if (!empty($id)) {
            $this->db->where('id', $id);
            $this->db->update('roads', ['sa_lot_no' => NULL]);
    ...
    {
        $this->model->back_to_inbox($id);
        $this->session->set_flashdata('message', array('type' => 'success', 'message' => 'Sent back to scheme inbox successfully'));
        redirect('ssm');
    }
    ...
    function not_traceable($id)
    {
        ...
        $this->data['heading'] = 'Scheme Not Implemented - Rural Roads(2025)';
    ...
    function not_traceable_save()
    {
        ...
        $result = $this->model->not_traceable_save($data);
        ...
        if ($result) {
            ...
            $this->session->set_flashdata('message', [
                'type'    => 'warning',
                'message' => 'Cannot be saved now, please try again later'
            ]);

            redirect('roads'); // <-- added fallback redirect (otherwise page stays blank)
        }
    }
    ...
    function scheme_not_implemented()
    {
        ...
        $this->data['title'] = 'Scheme Not Implemented';
        $this->data['heading'] = 'Not Implemented List - Rural Roads(2025)';
        $this->data['subheading'] = 'Scheme Not Implemented';
        $this->data['selected'] = json_encode($selected);
    ...
    function delete_not_traceable($id)
    {
        ...
        redirect('ssm/scheme_not_implemented');
    }
    ...
    function tender()
    {
        ...
        $this->data['ac'] = json_encode($this->model->get_ac_list());
        $this->data['block'] = json_encode($block_list);
        $this->data['approved'] = json_encode($this->model->get_tender_list($selected['district_id'], $selected['block_id'], $selected['ac_id']));
        $this->data['title'] = 'Tender';
        $this->data['heading'] = 'Tender - Rural Roads(2025)';
    ...
    {
        $data = $this->input->get();
        echo json_encode($this->model->get_tender_list($data['district_id'], $data['block_id'], $data['ac_id']));
    }
    ...
    function tender_benefitted_save()
    {
        ...
        if ($this->model->tender_benefitted_save($data)) {
            $this->session->set_flashdata('message', array('type' => 'success', 'message' => 'Tender Save successfully'));
            redirect('roads/tender_entry/' . $data['id']);
        } else {
            $this->session->set_flashdata('message', array('type' => 'warning', 'message' => 'can\'t save now, please try again later'));
            // echo '<script>alert("cannot be saved now, please try again later");</script>';
        }
    ...
    function wo()
    {
        ...
        $wo = $selected['district_id'] > 0 ? $this->model->get_wo_list($selected['district_id'], $selected['block_id'], $selected['ac_id']) : array();
        $this->data['role_id'] = $session['role_id'];
        $this->data['district'] = json_encode($district_list);
        $this->data['ac'] = json_encode($this->model->get_ac_list());
        $this->data['block'] = json_encode($block_list);
        ...
        $this->data['heading'] = 'Work Order - Rural Roads(2025)';
    ...
    function wo_save()
    {
        ...
        redirect(base_url('roads/wo'));
    }

    function get_wo_list()
    {
        $data = $this->input->get();
        echo json_encode($this->model->get_wo_list($data['district_id'], $data['block_id'], $data['ac_id']));
    }
    ...
    {
        $session = $this->common->get_session();
        if (in_array($session['role_id'], [1, 2, 3, 20])) {
            $wp_list = $this->model->get_wp_list($session['user_id'], $status);
            $this->data['wp'] = json_encode($wp_list);
            $this->data['status'] = $status;
            $this->data['heading'] = 'Work Progress List - Rural Roads(2025)';
    ...
    function wp_entry($id, $status)
    {
        ...
        $this->data['role_id'] = $session['role_id'];
        $this->data['status'] = $status;
        $this->data['heading'] = 'Work Progress Entry - Rural Roads(2025)';
        $this->data['subheading'] = $status == 0 ? 'Not-Started' : ($status == 5 ? 'Completed' : 'Work-In-Progress');
    ...
    function wp_save()
    {
        ...
        redirect(base_url('roads/wp/' . $input['status']));
    }

    // function wp_upload($id, $ssm_id)
    // {
    //     $path = 'uploads/roads/progress/';
    //     if (!file_exists('./' . $path)) {
    //         mkdir('./' . $path, 0777, true);
    //     }
    //     $config['upload_path'] = './' . $path;
    ...
    function wp_upload($id, $roads_id)
    {
        $path = 'uploads/roads/progress/';
        if (!file_exists('./' . $path)) {
            mkdir('./' . $path, 0777, true);
        ...
        if (!$this->upload->do_upload('image1')) {
            ...
            $old_file = $path . '/' . $file['upload_data']['file_name'];
            if (strlen($old_file) > 0) {
                $new_file = $path . $roads_id . '_' . $id . '_' . 'image1' . $file['upload_data']['file_ext'];
                rename($old_file, $new_file);
                $input = array(
                    'image1' => $new_file
                );
        ...
        if (!$this->upload->do_upload('image2')) {
            ...
            $old_file = $path . '/' . $file['upload_data']['file_name'];
            if (strlen($old_file) > 0) {
                $new_file = $path . $roads_id . '_' . $id . '_' . 'image2' . $file['upload_data']['file_ext'];
                rename($old_file, $new_file);
                $input = array(
                    'image2' => $new_file
                );
        ...
        if (!$this->upload->do_upload('image3')) {
            ...
            $old_file = $path . '/' . $file['upload_data']['file_name'];
            if (strlen($old_file) > 0) {
                $new_file = $path . $roads_id . '_' . $id . '_' . 'image3' . $file['upload_data']['file_ext'];
                rename($old_file, $new_file);
                $input = array(
                    'image3' => $new_file
                );
    ...
    function qm()
    {
        ...
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'Quality Monitoring - Rural Roads(2025)';
    ...
    function qm_details($id)
    {
        ...
        $this->data['heading'] = 'Assignment Details - Rural Roads(2025)';
    ...
    function qm_inspection()
    {
        ...
        $this->data['sqm'] = json_encode($sqm_list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'SQM Inspection - Rural Roads(2025)';
    ...
    function qm_start($id)
    {
        if ($id > 0) {
            ...
            $this->data['heading'] = 'Inspection Images - Rural Roads(2025)';
    ...
    function qm_save_start()
    {
        ...
        if ($id > 0) {
            ...
            $this->db->update('ROADS_QM_INSPECTION_IMAGE', $input);
        ...
        redirect(base_url('roads/qm_inspection'));
    }
    ...
    function qm_submit($id)
    {
        if ($id > 0) {
            ...
            $this->data['heading'] = 'Overall Quality Grading Report - Rural Roads(2025)';
    ...
    function qm_save_submit()
    {
        ...
        redirect(base_url('ssm/qm_inspection'));
    }
    ...
    function qm_atr()
    {
        ...
        $this->data['list'] = json_encode($list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'Action Taken Report - Rural Roads(2025)';
    ...
    function qm_inspection_report()
    {
        ...
        $this->data['sqm'] = json_encode($sqm_list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'Inspection Report - Rural Roads(2025)';
    ...
    function qm_inspection_report_view($id)
    {
        if ($id > 0) {
            ...
            $this->data['heading'] = 'Inspection Report View - Rural Roads(2025)';
    ...
    function report()
    {
        $this->data['heading'] = 'Report Overview - Rural Roads(2025)';
        $this->data['subheading'] = 'Rural Roads(2025) schemes';
        $this->data['title'] = 'Rural Roads(2025)';
    ...
    function rpt_state_summary()
    {
        ...
        $this->data['heading'] = 'State Summary Report - Rural Roads(2025)';
        $this->data['subheading'] = 'Rural Roads(2025) schemes';
        $this->data['title'] = 'Rural Roads(2025)';
    ...
    function rpt_agency_progress()
    {
        ...
        $this->data['heading'] = 'Agency wise Progress - Rural Roads(2025)';
        $this->data['subheading'] = 'Rural Roads(2025) schemes';
        $this->data['title'] = 'Rural Roads(2025)';
    ...
    function rpt_road_type_progress()
    {
        ...
        $this->data['list'] = json_encode($list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'Road Type wise Progress - Rural Roads (2025)';
        $this->data['subheading'] = 'Rural Roads (2025) schemes';
        $this->data['title'] = 'Rural Roads (2025)';
    ...
    function rpt_work_type_progress()
    {
        ...
        $this->data['list'] = json_encode($list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'Work Type wise Progress - Rural Roads (2025)';
        $this->data['subheading'] = 'Rural Roads (2025) schemes';
        $this->data['title'] = 'Rural Roads (2025)';
    ...
    function rpt_ps_work_status()
    {
        ...
        $this->data['selected'] = json_encode($selected);
        $this->data['list'] = json_encode($list);
        $this->data['heading'] = 'PS wise Work Status Report - Rural Roads (2025)';
        $this->data['subheading'] = 'Rural Roads (2025) schemes';
        $this->data['title'] = 'Rural Roads (2025)';
    ...
    function rpt_work_progress()
    {
        ...
        $this->data['selected'] = json_encode($selected);
        $this->data['list'] = json_encode($list);
        $this->data['heading'] = 'Work wise Progress Report - Rural Roads (2025)';
        $this->data['subheading'] = 'Rural Roads (2025) schemes';
        $this->data['title'] = 'Rural Roads (2025)';
    ...
    function rpt_work_progress_details($roads_id)
    {
        $list = $this->model->get_rpt_work_progress_details($roads_id);
        $this->data['list'] = sizeof($list) > 0 ? json_encode($list) : '';
        $this->data['title'] = 'Work Progress Report';
        $this->data['heading'] = 'Work Progress Details Report - Rural Roads(2025)';
    ...
    function rpt_qm_summary()
    {
        ...
        $this->data['district'] = json_encode($district_list);
        $this->data['heading'] = 'Quality Monitoring Report - Rural Roads(2025)';
        $this->data['subheading'] = 'Rural Roads(2025) schemes';
        $this->data['title'] = 'Rural Roads(2025)';
    ...
    function update()
    {
        ...
        $this->data['selected'] = json_encode($selected);
        $this->data['list'] = json_encode($list);
        $this->data['heading'] = 'Update of Pathashree Schemes - Rural Roads(2025)';
        $this->data['subheading'] = 'Rural Roads(2025) schemes';
        $this->data['title'] = 'Rural Roads(2025)';
    ...
    function rpt_update_summary()
    {
        ...
        $this->data['selected'] = json_encode($selected);
        $this->data['list'] = json_encode($list);
        $this->data['heading'] = 'Update Summary Report - Rural Roads(2025)';
        $this->data['subheading'] = 'Rural Roads(2025) schemes';
        $this->data['title'] = 'Rural Roads(2025)';
    ...
    function rpt_approval_progress()
    {
        $selected = array(
            'ac_id' => ''
        );
        ...
        $this->data['ac'] = json_encode($this->model->get_ac_list());
        $this->data['selected'] = json_encode($selected);
        $this->data['list'] = json_encode($this->model->get_rpt_approval_progress($selected['ac_id']));
        $this->data['heading'] = 'Approval Progress - Rural Roads(2025)';
        $this->data['subheading'] = 'Rural Roads(2025) schemes';
        $this->data['title'] = 'Rural Roads(2025)';
        $this->data['content'] = $this->parser->parse('rpt_approval_progress', $this->data, true);
        $this->parser->parse('../../templates/template.php', $this->data);
    }
    function rpt_temp_details($flag = 3)
    {
        switch ($flag) {
            case 2:
                $list = $this->model->get_rpt_temp_srrp();
                break;
            case 3:
                $list = $this->model->get_rpt_temp_ssm();
                break;
        }
        // if($flag == 2) {
        //     $list = $this->model->get_rpt_temp_ssm();
        ...
        //     $list = $this->model->get_rpt_temp_srrp();
        // }
        //$list = $this->model->get_rpt_temp_details();
        $this->data['list'] = sizeof($list) > 0 ? json_encode($list) : '';
        $this->data['title'] = $flag == 3 ? 'Temp Progress Report Pathashree-III' : 'Temp Progress Report Pathashree-II';
        $this->data['heading'] = 'Temp Details Report';
        $this->data['subheading'] = $flag == 3 ? 'Temp Progress Report Pathashree-III' : 'Temp Progress Report Pathashree-II';
        $this->data['content'] = $this->parser->parse('rpt_temp', $this->data, true);
        $this->parser->parse('../../templates/template.php', $this->data);
    }

    function update_completion_date()
    {
        $data = $this->input->get();
        echo json_encode($this->model->update_completion_date($data));
        ...
        if ($complete_date > $start_date) {
            $this->session->set_flashdata('message', array('type' => 'success', 'message' => 'Date save successfully'));
        } elseif ($complete_date == $start_date) {
            $this->session->set_flashdata('message', array('type' => 'warning', 'message' => 'Complete date not equal start date'));
        } else {
            $this->session->set_flashdata('message', array('type' => 'danger', 'message' => 'Enter date grater then start date'));
        }
    }
    function update_wo_date()
    {
        $data = $this->input->get();
        // print_r($data); exit;
        echo json_encode($this->model->update_wo_date($data));
        if ($data['id'] > 0) {
            $this->session->set_flashdata('message', array('type' => 'success', 'message' => 'Date save successfully'));
        }
    }
    function rpt_agency_wise_updated()
    {
        $list = array();
        $selected = array(
            'district_id' => '',
        ...
        if ($this->input->post()) {
            ...
            if (sizeof($_post) > 0) {
                ...
                $list = $this->model->rpt_agency_wise_updated($selected['district_id'], $selected['agency'], $selected['status_id']);
            }
        }
        $district_list = $this->common->get_district_list();
        if (sizeof($district_list) == 1) {
        ...
        $this->data['district'] = json_encode($district_list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'Agency Wise Updated Details';
        $this->data['subheading'] = 'Rural Roads(2025) schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
        $this->data['content'] = $this->parser->parse('rpt_agency_wise_updated', $this->data, true);
        $this->parser->parse('../../templates/template.php', $this->data);
    }
    function rpt_updated_work_details()
    {
        $block_list = array();
        $list = array();
        $selected = array(
        ...
        if ($this->input->post()) {
            ...
            if (sizeof($_post) > 0) {
                ...
                $block_list = $this->common->get_block_list($selected['district_id']);
                $list = $this->model->get_rpt_updated_work_details($selected['district_id'], $selected['block_id'], $selected['agency'], $selected['status_id']);
            }
        }
        $district_list = $this->common->get_district_list();
        if (sizeof($district_list) == 1) {
        ...
        $this->data['district'] = json_encode($district_list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'Updated Work Details';
        $this->data['subheading'] = 'RURAL_ROADS(2025) schemes';
        $this->data['title'] = 'RURAL ROADS(2025)';
        $this->data['content'] = $this->parser->parse('rpt_updated_work_details', $this->data, true);
        $this->parser->parse('../../templates/template.php', $this->data);
    }
    function rpt_updated_work_summary()
    {
        $block_list = array();
        $list = array();
        $selected = array(
        ...
        if ($this->input->post()) {
            ...
            if (sizeof($_post) > 0) {
                ...
                $block_list = $this->common->get_block_list($selected['district_id']);
                $list = $this->model->get_rpt_updated_work_summary($selected['district_id'], $selected['block_id'], $selected['agency']);
            }
        }
        $district_list = $this->common->get_district_list();
        if (sizeof($district_list) == 1) {
        ...
        $this->data['district'] = json_encode($district_list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'Updated Work Summary';
        $this->data['subheading'] = 'Rural Roads(2025) schemes';
        $this->data['title'] = 'RURAL_ROADS(2025)';
        $this->data['content'] = $this->parser->parse('rpt_updated_work_summary', $this->data, true);
        $this->parser->parse('../../templates/template.php', $this->data);
    }
    function date_update()
    {
        $data = $this->input->post();
        // print_r($data); exit;
        echo json_encode($this->model->date_update($data));
    }

    function rpt_all_documnets()
    {
        $block_list = array();
        $list = array();
        $selected = array(
        ...
        if ($this->input->post()) {
            ...
            if (sizeof($_post) > 0) {
                ...
                $block_list = $this->common->get_block_list($selected['district_id']);
                $list = $this->model->get_rpt_all_documents($selected['district_id'], $selected['block_id'], $selected['scheme_id']);
            }
        }
        $district_list = $this->common->get_district_list();
        if (sizeof($district_list) == 1) {
        ...
        $this->data['district'] = json_encode($district_list);
        $this->data['selected'] = json_encode($selected);
        $this->data['heading'] = 'All Documents';
        $this->data['subheading'] = 'PATHASHREE-III AND PATHASHREE-II schemes';
        $this->data['title'] = 'PATHASHREE-RASTASHREE';
        $this->data['content'] = $this->parser->parse('rpt_all_documents', $this->data, true);
        $this->parser->parse('../../templates/template.php', $this->data);
    }
}
