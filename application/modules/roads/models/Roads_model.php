...
class Roads_Model extends CI_Model
{
    ...
    function get_dashboard_count()
    {
        ...
        SUM(CASE WHEN s.tender_status >= 1 THEN 1 ELSE 0 END) AS tender_invited,
    ...
    function get_survey_list($district_id, $status = 0)
    {
        ...
        $this->db->select('s.id, d.name as district, '
            ...
            . 's.survey_estimated_doc, s.survey_lot_no, s.survey_lot_doc, s.dm_lot_no, s.dm_lot_doc, s.se_lot_no, s.se_lot_doc, s.sa_lot_no, s.admin_approval_doc, s.isactive, ni.remarks, ni.status_id');
        $this->db->where(array(
    ...
    function save($data)
    {
        ...
        $input = array(
            ...
            'paver_length'     => ($data['paver_length'] === '' ? null : $data['paver_length']),
        );
    ...
    ");
        ...
        // Group and order
        $this->db->group_by(['s.district_id','s.block_id', 's.agency', 's.work_type']);
        $this->db->order_by('d.name', 'ASC');
