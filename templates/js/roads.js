...
function get_block_list() {
  $.ajax({
    ...
    if (data.length > 0) {
      $('#block_id').append(
        $('<option>', { value: '0', text: '--All ULB Name--' })
      );
      $.each(data, function (i, item) {
        $('#block_id').append(
      ...
      $('#block_id').append(
        $('<option>', { value: '', text: '--Select ULB Name--' })
      );
    }
  });
}
...
function get_gp_list() {
  ...
  $.ajax({
    ...
    if (data.length > 0) {
      $('#gp_id').append(
        $('<option>', { value: '', text: '--Select Ward No--' })
      );
      $.each(data, function (i, item) {
        ...
        render: function (data, type, row, meta) {
          ...
          if (row.survey_estimated_doc) {
            html +=
              ...
              '" target="_blank" ' +
              'style="display:inline-block; margin:5px; padding:5px 10px; font-size:12px; font-weight:500; color:#fff; background:#4a90e2; border-radius:6px; text-decoration:none;">' +
              '<i class="fa fa-file-alt"></i> View Doc</a>';
          }

          // Upload button
          if (role_id < 3) {
            html +=
              '<button class="btn-upload" data-id="' +
              row.id +
              '" ' +
              'style="display:inline-block; margin:5px; padding:5px 10px; font-size:12px; font-weight:500; color:#fff; background:#28a745; border:none; border-radius:6px; cursor:pointer;">' +
...
$(document).on('click', '.btn-upload', function () {
  ...
  fileInput.on('change', function () {
    ...
    if (file.size > maxSize) {
      alert('File size exceeds 5 MB limit. Please upload a smaller PDF.');
    ...
    $.ajax({
      url: baseURL + '/roads/survey_upload_manage/' + id,
      type: 'POST',
      data: formData,
      processData: false,
      contentType: false,
      success: function (response) {
        if (response.status) {
          ...
          $('#yourTable').DataTable().ajax.reload(null, false);
          setTimeout(function () {
            location.reload(); // remove this line if you don't want full reload
          }, 1000);
        } else {
      ...
      complete: function () {
        fileInput.remove(); // Clean up hidden input after upload
      },
    });
  });
});
